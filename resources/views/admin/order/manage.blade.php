                    ...
                    <table id="datatable" class="table table-bordered  " >
                        ...
                        @foreach($orders as $order)
                            <tr>
                                ...
                                <td class="h4"><span class="{{$order->order_status == 'Complete' ? "badge bg-success text-white" : "badge bg-warning text-white"}}">{{$order->order_status}}</span></td>
                                <td>{{$order->order_date}}</td>
                                <td>{{$order->payment_method == 1 ? 'Cash On Delivery' : 'Online Payment'}}</td>
                                <!--<td>{{$order->payment_status }}</td>-->
                                <td class="h4"><span class="{{$order->payment_status == 'Complete' ? "badge bg-success text-white" : "badge bg-warning text-white"}}">{{$order->payment_status}}</span></td>
                                <td>
